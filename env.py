...
class Connect4(object):
	...
	def __init__(self, n_rows=6, n_columns=7, win_streak=4):
		...
		h_win_kernel = np.zeros(shape=(1, win_streak)) # horizontal win
		v_win_kernel = np.zeros(shape=(win_streak, 1)) # vertical win
	...
	def check_win(self, player_id):
		...
		for kernel in self.win_kernels:
			...
			win_mask = scipy.signal.convolve2d(self.grid, kernel, mode="full")
